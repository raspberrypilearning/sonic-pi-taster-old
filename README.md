...
So far you have used a simple sequence and repeating loops to create your music. Every now and then it might be fun to add an element of surprise. You can achieve this in your code by setting a condition.
...
Every time round this loop both notes will play in sequence five times. 
...
2. Type the following code:
  ...
  You have a set a condition. The condition states that if the random number generated by `rand` (which stands for 	  random) is less than 0.5 then `play 64` if the number generated is greater than 0.5 then `play 89`. As every 		  time around the loop a random number is generated there may be a different note played each time. 
	

3. Add a conditional to your own tune somewhere to add an element of surprise. 
